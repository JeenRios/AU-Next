    ...
    <>
      ...
      <div className="col-span-12 lg:col-span-8 space-y-6">
        ...
        <div className="bg-white rounded-3xl border border-gray-100 shadow-sm p-6">
          <div className="flex gap-4">
            <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-primary-gold to-secondary-gold flex items-center justify-center text-white font-bold text-lg flex-shrink-0 shadow-sm">
            ...
            <div className="flex-1">
              <textarea
                ...
                className="w-full p-4 bg-gray-50/50 rounded-2xl border border-gray-100 focus:ring-2 focus:ring-2 focus:ring-primary-gold/20 focus:border-primary-gold focus:outline-none transition-all resize-none h-28 font-medium text-sm"
              />
              <div className="flex items-center justify-between mt-4">
                <div className="flex gap-1">
                  <button className="p-2.5 text-gray-400 hover:text-primary-gold hover:bg-primary-gold/10 rounded-xl transition-all">
                ...
                <button
                  ...
                  className="px-8 py-2.5 bg-surface-dark hover:bg-surface-dark/90 text-white font-bold rounded-xl transition-all disabled:opacity-50 shadow-sm"
                >
                  Share Post
                </button>
              </div>
        ...
        {posts.map((post) => (
          <div key={post.id} className="bg-white rounded-3xl border border-gray-100 shadow-sm hover:shadow-md transition-all overflow-hidden">
            ...
            <div className="p-6 flex items-center gap-4">
              <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-primary-gold/10 to-secondary-gold/10 border border-primary-gold/20 flex items-center justify-center text-primary-gold font-bold shadow-sm">
                {post.user.avatar}
              </div>
              <div className="flex-1">
                <div className="flex items-center gap-2">
                  <span className="font-bold text-surface-dark">{post.user.name}</span>
                  {post.user.verified && (
                  ...
                  {post.user.id !== user?.id && (
                    <>
                      ...
                      <button
                        ...
                        className={`text-xs font-bold transition-colors ${post.user.following ? 'text-gray-400' : 'text-primary-gold hover:text-primary-gold/90'}`}
                      >
            ...
            <div className="px-6 pb-6">
              <p className="text-surface-dark text-base leading-relaxed font-medium whitespace-pre-wrap">{post.content}</p>
              {post.image && (
            ...
            <div className="px-6 py-4 bg-gray-50/50 border-t border-gray-50 flex items-center gap-8">
              ...
              <button
                ...
                className={`flex items-center gap-2 transition-all ${activeComments === post.id ? 'text-primary-gold' : 'text-gray-400 hover:text-primary-gold'}`}
              >
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <button className="flex items-center gap-2 text-gray-400 hover:text-primary-gold transition-colors">
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            {activeComments === post.id && (
              <div className="px-6 py-6 bg-gray-50/30 border-t border-gray-50 space-y-6 animate-in slide-in-from-top-4 duration-300">
                ...
                <div className="space-y-4">
                  {postComments[post.id]?.map((comment) => (
                    <div key={comment.id} className="flex gap-3">
                      <div className="w-8 h-8 rounded-lg bg-white border border-gray-100 flex items-center justify-center text-primary-gold text-xs font-bold flex-shrink-0 shadow-sm">
                        {comment.avatar}
                      </div>
                      <div className="flex-1 bg-white p-3 rounded-2xl rounded-tl-none border border-gray-100 shadow-sm">
                        <div className="flex items-center justify-between mb-1">
                          <span className="text-xs font-bold text-surface-dark">{comment.user_name}</span>
                ...
                <div className="flex gap-3 pt-2">
                  <div className="w-8 h-8 rounded-lg bg-surface-dark flex items-center justify-center text-white text-xs font-bold flex-shrink-0">
                    {user?.name?.charAt(0) || 'U'}
                  </div>
                  <div className="flex-1 flex gap-2">
                    <input
                      ...
                      className="flex-1 px-4 py-2 bg-white border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-2 focus:ring-primary-gold/20 focus:border-primary-gold text-sm font-medium transition-all"
                    />
                    <button
                      onClick={() => handleAddComment(post.id)}
                      disabled={!newComment.trim()}
                      className="px-4 bg-primary-gold text-white rounded-xl hover:bg-primary-gold/90 transition-colors disabled:opacity-50"
                    >
        ...
        {posts.length === 0 && !loading && (
          <EmptyState
            icon={<svg className="w-8 h-8 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 20H5a2 2 0 01-2-2V6a2 2 0 012-2h10l4 4v10a2 2 0 01-2 2z" /></svg>}
      ...
      <div className="col-span-12 lg:col-span-4 space-y-6">
        ...
        <div className="bg-white rounded-3xl border border-gray-100 shadow-sm p-6 sticky top-8">
          <h3 className="font-bold text-surface-dark mb-6 flex items-center gap-3">
          ...
          <div className="space-y-4">
            {leaderboard.map((trader, idx) => (
              <div key={idx} className={`flex items-center gap-4 p-3 rounded-2xl transition-all border ${
                idx === 0 ? 'bg-gradient-to-br from-primary-gold/10 to-secondary-gold/10 border-primary-gold/20 shadow-sm' : 'border-transparent hover:bg-gray-50'
              }`}>
                <div className={`w-8 h-8 rounded-xl flex items-center justify-center font-bold text-xs shadow-sm ${
                  idx === 0 ? 'bg-primary-gold text-white' :
                  idx === 1 ? 'bg-slate-300 text-white' :
                  idx === 2 ? 'bg-primary-gold text-white' : 'bg-gray-100 text-gray-500'
                }`}>
                  {trader.rank}
                </div>
                <div className="flex-1">
                  <div className="font-bold text-surface-dark text-sm truncate max-w-[120px]">{trader.name}</div>
          ...
          <button className="w-full mt-6 py-3 border border-gray-100 text-gray-500 hover:text-primary-gold hover:bg-primary-gold/10 hover:border-amber-100 font-bold rounded-2xl transition-all text-xs">
          ...
          <div className="space-y-4">
            {suggested.map((trader) => (
              <div key={trader.id} className="flex items-center gap-4 p-2 group">
                ...
                <div className="flex-1">
                  <div className="font-bold text-surface-dark text-sm flex items-center gap-1.5">
                    {trader.name}
                ...
                <button
                  ...
                  className={`px-4 py-1.5 rounded-xl text-xs font-bold transition-all shadow-sm ${
                    trader.following
                      ...
                      : 'bg-surface-dark text-white hover:bg-surface-dark/90 hover:-translate-y-0.5'
                  }`}
                >
          ...
          <div className="space-y-2">
            {['#XAUUSD', '#GoldBullRun', '#CPI_Data', '#ForexSignals', '#TradingPsychology'].map((topic, idx) => (
              <button key={idx} className="w-full text-left px-4 py-3 hover:bg-gray-50 rounded-2xl transition-all border border-transparent hover:border-gray-100 group">
                <div className="font-bold text-primary-gold text-sm group-hover:translate-x-1 transition-transform">{topic}</div>
