  ...
  const handleMT5Submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data?.success) {
        ...
        setMt5Accounts([...mt5Accounts, newAccount]);
        ...
        showToast(data?.message || 'Failed to request MT5 connection.', 'error');
      }
    } catch (err) {
              ...
              <div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  {mt5Accounts.map((account) => (
                    <div key={account.id} className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6">
                      ...
                      <div className="grid grid-cols-3 gap-4 mb-4 p-4 bg-gray-50 rounded-xl">
                        <div>
                          ...
                          <div className="font-bold text-[#1a1a1d]">${account.balance.toFixed(2)}</div>
                        </div>
                        <div>
                          <div className="text-xs text-gray-600 mb-1">Equity</div>
                          <div className="font-bold text-[#1a1a1d]">${account.equity.toFixed(2)}</div>
                        </div>
                        <div>
                          <div className="text-xs text-gray-600 mb-1">Profit</div>
                          <div className={`font-bold ${account.profit >= 0 ? 'text-green-600' : 'text-red-600'}`}>
